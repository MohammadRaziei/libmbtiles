...
Down-sample zoom levels from an MBTiles archive. The command can write the
resulting tiles to a directory (using a filename pattern) or directly into a new
MBTiles file.

```
mbtiles-cli decrease-zoom <mbtiles> <output> [--levels ...] [-p <pattern>] [-y] [-v] [--grayscale]
```

- `<mbtiles>` – Source MBTiles file. Must exist.
- `<output>` – Destination directory **or** a path ending in `.mbtiles`.
- `--levels` – List of zoom levels to include. Negative values are relative to
  the maximum zoom (for example `-1` generates the next zoom down from the
  highest level). Omit to generate just `max-1`.
...
`--levels` are copied as-is while any relative levels are generated by
down-sampling higher zooms. The output metadata is updated to reflect the new
minimum and maximum zooms.
