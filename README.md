...
Resize tiles to generate new zoom levels or copy existing ones from an MBTiles
archive. The command can write the resulting tiles to a directory (using a
...
- `<output>` – Destination directory **or** a path ending in `.mbtiles`.
- `--levels` – List of zoom levels to include. Prefix values with `-` to request
  zoom levels below the minimum (e.g. `-1` generates `min-1`) or with `+` to
  request levels above the maximum (e.g. `+1` generates `max+1`). Unprefixed
  values are treated as absolute zoom numbers. Omit the option to generate a
  single level at `min-1`.
- `-p`, `--pattern` – Filename pattern used when writing to a directory. The
...
When writing to another MBTiles file, existing zoom levels listed via
`--levels` are copied as-is while new levels are synthesized by down-sampling or
up-sampling neighbouring zoom levels. The output metadata is updated to reflect
the resulting minimum and maximum zoom values.
...
```cpp
void resize_zoom_levels(const std::string &input_mbtiles,
                        const std::string &output_path,
                        const ResizeOptions &options = {});
```

`ResizeOptions` fields:

- `target_levels` – Zoom levels to include in the output. When empty the
  function generates the level directly below the minimum zoom present in the
  source archive.
...
child tiles (down-sampling) or splitting a tile into four children (up-sampling)
with `stb_image_resize2`. Results can be written to a directory structure or to
a new MBTiles database.
